...
same_identity = IF( OR('female (?x)', 'male (?x)'), THEN ('same-identity (?x) (?x)'))
sibling = IF( AND('parent (?x) (?y)', 'parent (?x) (?z)', NOT('same-identity (?y) (?z)')), THEN('sibling (?z) (?y)', 'sibling (?y) (?z)'))
mother = IF( AND('female (?x)', 'parent (?x) (?y)'), THEN('mother (?x) (?y)'))
father = IF( AND('male (?x)', 'parent (?x) (?y)'), THEN('father (?x) (?y)'))
son = IF( AND('male (?y)', 'parent (?x) (?y)'), THEN('son (?y) (?x)'))
daughter = IF( AND('female (?y)', 'parent (?x) (?y)'), THEN('daughter (?y) (?x)'))
sister = IF( AND('female (?x)', 'sibling (?x) (?y)'), THEN('sister (?x) (?y)'))
brother = IF( AND('male (?x)', 'sibling (?x) (?y)'), THEN('brother (?x) (?y)'))
cousin = IF( AND('sibling (?x) (?y)', 'parent (?x) (?a)', 'parent (?y) (?b)'), THEN('cousin (?a) (?b)'))
grands = IF ( AND('parent (?x) (?y)', 'parent (?y) (?z)'), THEN('grandparent (?x) (?z)', 'grandchild (?z) (?x)'))

# Then, put them together into a list in order, and call it
# family_rules.
family_rules = [same_identity, sibling, father, mother, sister, brother, son, daughter, cousin, grands ]                    # fill me in
